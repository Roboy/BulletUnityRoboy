...
public class BulletSenseGlove : MonoBehaviour
{
    ...
    void trackFingerJointsSim()
    {
        ...
        var cmd = NativeMethods.b3JointControlCommandInit2(pybullet, robot.Getb3RobotId(), (int)EnumControlMode.CONTROL_MODE_POSITION_VELOCITY_PD);
        ...
        for (int i = 0; i < gloveInfo.handJointsIds.Count; i++)
        {
            ...
            for (int j = 0; j < gloveInfo.handJointsIds[i].Count; j++)
            {
                ...
                bb.SetJointPosition(ref cmd, robot.Getb3RobotId(), gloveInfo.handJointsIds[i][j], targetJointPos);
            }
        }
